...
class PACRR(MODEL_BASE):
    ...
    def build_bi_rnns(self):
        q_bi_rnn = Bidirectional(LSTM(self.p['hidden_size'], return_sequences=True), merge_mode='concat', input_shape=(self.p['maxqlen'], self.p['emb_dim']))
        d_bi_rnn = Bidirectional(LSTM(self.p['hidden_size'], return_sequences=True), merge_mode='concat', input_shape=(self.p['simdim'], self.p['emb_dim']))
        return q_bi_rnn, d_bi_rnn

    def build_common_bi_rnn(self):
        bi_rnn = Bidirectional(LSTM(self.p['hidden_size'], return_sequences=True), merge_mode='concat', input_shape=(self.p['simdim'], self.p['emb_dim']))
        return bi_rnn
